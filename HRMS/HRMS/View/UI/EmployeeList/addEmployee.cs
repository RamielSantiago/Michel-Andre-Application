...
{
    public partial class addEmployee : UserControl, iRegister
    {
        ...
        private void addEmpBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please fill all required fields", "Human Resource Management System", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                valid = false;
            }
            else
            {
                ...
                if (LRA.RP.SearchUser("Username", UserName) == 1)
                {
                    valid = false;
                ...
                if (LRA.RP.SearchName(FirstName, LastName) == -1)
                {
                    valid = false;
                    errorMsg = errorMsg + "This Full Name is taken." + Environment.NewLine;
                }
                if (valid == true)
                {
                    LRA.RP.AddUser();
